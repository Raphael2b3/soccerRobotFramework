...
TEST_CASE("Higher event priority wins regardless of sender") {
    ...
    auto e1 = std::make_shared<BaseEvent>(AgentId::getNewId("test"), 5);
    auto e2 = std::make_shared<BaseEvent>(AgentId::getNewId("test"), 3);
...
TEST_CASE("Equal event priority, higher sender priority wins") {
    ...
    auto e1 = std::make_shared<BaseEvent>(id1, 3);
    auto e2 = std::make_shared<BaseEvent>(id2, 3);
...
TEST_CASE("Equal event and sender priority = no higher priority") {
    ...
    auto e1 = std::make_shared<BaseEvent>(AgentId::getNewId("test"), 3);
    auto e2 = std::make_shared<BaseEvent>(AgentId::getNewId("test"), 3); // same sender, same prio
...
TEST_CASE("Default sender priority applies when not set") {
    ...
    auto e1 = std::make_shared<BaseEvent>(AgentId::getNewId("test"), 2);
    auto e2 = std::make_shared<BaseEvent>(AgentId::getNewId("test"), 2);
...
TEST_CASE("Null event throws exception") {
    Priority prio;
    auto dummy = std::make_shared<BaseEvent>(AgentId::getNewId("test"), 1);
